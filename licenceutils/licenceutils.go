...
func CheckLicense() {
	...
	if !FileExist(fileName) {
		fmt.Printf("%s:", menuutils.Cyan("Lincense file not found"))
		...
		if isLicenceValid {
			...
			ShowPaymentQr()
			fmt.Println()
			fmt.Printf("%s: %s\n", menuutils.Cyan("Send 1 ETH To"), menuutils.Yellow("0x6d5F00aE01F715D3082Ad40dfB5c18A1a35d3A17"))
			fmt.Printf("%s\n", menuutils.Cyan("You will receive email with API KEY after our system process payment"))
			fmt.Println()
		}
	}
}
